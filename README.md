...
- **DNS / SSL / ICMP tests** — No Checkly equivalent yet (TCP is supported as of 2026 via `TcpMonitor`)
- **TCP / DNS / ICMP steps inside multi-step tests** — Playwright has no equivalent for these step types. The surrounding multi-step test is skipped; standalone TCP synthetics are migrated separately by step 04b.
...
| `npm run migrate:api` | Convert API tests to ApiCheck (also runs `generate:tcp`) |
