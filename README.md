...
| TCP Tests (`subtype: tcp`) | TcpMonitor | Hostname, port, `responseTime` assertion → `maxResponseTime` |
| DNS Tests (`subtype: dns`) | DnsMonitor | Host → query, dnsServer → nameServer, `recordSome is V` → `textAnswer().contains(V)` |
...
│   ├── tcp/{public,private}/       # TcpMonitor constructs (inline mode; see TCP section below)
│   ├── dns/{public,private}/       # DnsMonitor constructs (inline mode; see DNS section below)
...
| `npm run migrate:api` | Convert API tests to ApiCheck (also runs `generate:tcp` and `generate:dns`) |
| `npm run generate:tcp` | Convert TCP synthetic tests (`subtype: tcp`) to TcpMonitor constructs |
| `npm run generate:dns` | Convert DNS synthetic tests (`subtype: dns`) to DnsMonitor constructs |
...
Datadog TCP synthetic tests (`subtype: tcp`) migrate to Checkly's `TcpMonitor` construct. The conversion is 1:1:
...
TCP files are generated by step 04b (`npm run generate:tcp`), which runs automatically as part of `npm run migrate:api`.
...
# Standalone mode: writes checkly-migrated/acme-tcp/ as a separate project
...
Datadog DNS synthetic tests (`subtype: dns`) migrate to Checkly's `DnsMonitor` construct. Conversion is 1:1 for the request shape and most assertions:

| Datadog | Checkly |
|---|---|
| `config.request.host` | `request.query` |
| `config.request.dnsServer` (if set & non-empty) | `request.nameServer` |
...
- [TCP Monitor Migration](migration_readmes/migration_tcp.md)
- [DNS Monitor Migration](migration_readmes/migration_dns.md)
