...
/**
 * Filter the source migration-mapping.csv down to just TCP rows.
 * Header row is preserved as-is. Returns the new CSV string, or null if
 * the source CSV doesn't exist (caller writes a header-only file in that case).
 */
async function filterMappingCsvForTcp(sourceCsvPath: string): Promise<string | null> {
  if (!existsSync(sourceCsvPath)) return null;
  const raw = await readFile(sourceCsvPath, 'utf-8');
  const lines = raw.split('\n');
  if (lines.length < 2) return null;
  const header = lines[0];
  const headerFields = header.split(',');
  const typeIdx = headerFields.indexOf('check_type');
  if (typeIdx === -1) return null;
  // Naive CSV row scan: tcp rows are always emitted unquoted by step 12.
  const tcpRows = lines.slice(1).filter(line => {
    if (!line.trim()) return false;
    // Quick check: split-then-locate works because step 12 emits tcp,public/private
    // with no commas in surrounding fields; but be defensive and scan with a regex.
    return new RegExp(`,tcp,`).test(line);
  });
  return [header, ...tcpRows].join('\n') + (tcpRows.length > 0 ? '\n' : '');
}
...
async function writeStandaloneScaffolding(
  ...
  // migration-mapping.csv — filter source down to TCP rows.
  const sourceMappingCsv = path.join(sourceRoot, 'migration-mapping.csv');
  const tcpCsv = await filterMappingCsvForTcp(sourceMappingCsv);
  if (tcpCsv) {
    await writeIfMissing('migration-mapping.csv', tcpCsv);
  } else {
