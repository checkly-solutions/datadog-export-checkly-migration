...
const MANIFEST_FILE = './checkly-migrated/tests/browser/_manifest.json';
const OUTPUT_DIR = './checkly-migrated/__checks__/browser';
 ...
 */
function generateBrowserCheckCode(test, specFilename) {
...
async function main() {
  ...
  if (!existsSync(INPUT_FILE)) {
    ...
    console.error('Run "npm run export" first to export browser tests.');
    process.exit(1);
  }

  if (!existsSync(MANIFEST_FILE)) {
    console.error(`Error: Manifest file not found: ${MANIFEST_FILE}`);
    console.error('Run "npm run generate:browser-specs" first to generate spec files.');
    process.exit(1);
  }
  ...
  for (const test of tests) {
    ...
    try {
      const code = generateBrowserCheckCode(test, specFilename);
