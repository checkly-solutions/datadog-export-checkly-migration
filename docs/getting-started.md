...
|---|---|---|---|
| **API tests** | → `ApiCheck`, full support | `OPTIONS`-method requests aren't converted by the tool yet; re-add them by hand on the generated `ApiCheck` | None |
| **Browser tests** | → `BrowserCheck` + Playwright `.spec.ts` | Locators/selectors from Datadog's multiLocator may need review (a primary review-time driver) | None |
| **Multi-step API tests** | → `MultiStepCheck` + Playwright `.spec.ts` | Variable extraction between steps may need manual edits (the other primary review-time driver) | A multi-step test containing TCP/DNS/ICMP steps is skipped entirely and recorded in the spec folder's `_manifest.json`; those steps can be recreated as standalone Checkly uptime monitors |
| **TCP tests** (`subtype: tcp`) | → `TcpMonitor` (step 04b) | None | None |
...
| **Global variables** | Non-secret → `variables/env-variables.json` (with values) | Secret → `variables/secrets.json` with values **empty**. Datadog never exports secret values; you fill them in before deploy | None |
...
| **Tags** | → tags (filterable via `DD_TAGS_*`) | None | None |
| **SSL / ICMP tests** | None | None | No Checkly equivalent yet |

A few advanced assertion types may need a manual follow-up; your AI assistant and the Checkly docs can help you close those.

Browser and multi-step are where the review time goes. If a locator didn't translate or a variable extraction came out wrong, your AI assistant is the fastest fix; see `troubleshooting.md` if you get stuck.
...
If the export comes back empty, check the App Key scopes first; that's the most common cause.
...
Browser locators and multi-step variable extraction are the two places review time concentrates. Your AI assistant is the first stop for both. Escalation order: AI assistant → this guide and `troubleshooting.md` → the [Checkly docs](https://www.checklyhq.com/docs/), then Checkly support if you're still stuck.
